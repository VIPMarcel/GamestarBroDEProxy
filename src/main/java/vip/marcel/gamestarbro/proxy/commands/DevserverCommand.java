...
public class DevserverCommand extends Command {
    ...
    public DevserverCommand(Proxy plugin, String name, String permission) {
    ...
    public void execute(CommandSender sender, String[] arguments) {
        ...
        if(sender instanceof ProxiedPlayer player) {
            ...
            final ServerInfo serverInfo = ProxyServer.getInstance().getServerInfo(this.plugin.getDevServerName());

            player.connect(serverInfo);
            player.sendMessage(this.plugin.getPrefix() + "Du wirst auf den §eDeveloper Server §7verschoben.");
